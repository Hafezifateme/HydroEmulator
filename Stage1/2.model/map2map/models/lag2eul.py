...
def lag2eul(*xs, up_fac = 1, squeeze=False, rm_dis_mean=True, periodic=False):
    """
    ...
    for x in xs:
        ...
        pos[:, 0] += torch.arange(0., DHW[0], up_fac, device=x.device)[:, None, None]
        pos[:, 1] += torch.arange(0., DHW[1], up_fac, device=x.device)[:, None]
        pos[:, 2] += torch.arange(0., DHW[2], up_fac, device=x.device)

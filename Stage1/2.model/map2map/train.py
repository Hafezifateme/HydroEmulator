...
def gpu_worker(local_rank, node, args):
    ...
    if args.adv:
        ...
        inchannel = sum(args.out_chan)
        if args.cgan:
            inchannel += sum(args.in_chan)
            if args.lag2eul:
                inchannel += 2 # one from in-eul and one from output-eul
                inchannel += 1 # cat eul-u field
                if args.mesh_up_fac>1:
                    inchannel += (args.mesh_up_fac**3)-1
            
        adv_model = adv_model(inchannel,1,scale_factor=args.scale_factor)
...
def train(epoch, loader, model, criterion, optimizer, scheduler,
    ...
    for i, (input, target) in enumerate(loader):
        ...
        if batch == 1 and rank == 0:
            print('narrowed input shape :', input.shape, flush=True)
            print('narrowed output shape :', output.shape, flush=True)
            print('narrowed target shape :', target.shape, flush=True)
        ...
        if args.adv and epoch >= args.adv_start:
            ...
            if args.lag2eul:
                lag_in, lag_out, lag_tgt = input, output, target
                eul_in, eul_out, eul_tgt = lag2eul(lag_in[:,:3], lag_out[:,:3], lag_tgt[:,:3], up_fac = args.mesh_up_fac, squeeze=True,rm_dis_mean=False)
                
                output = torch.cat([eul_out, output], dim=1)
                target = torch.cat([eul_tgt, target], dim=1)
                input = torch.cat([eul_in,input],dim=1)
                
                # concatenate eul-u field 
                u_ave = 2.68
                lag_out[:,3:4] = torch.pow(10,(lag_out[:,3:4].clone()+u_ave))
                lag_tgt[:,3:4] = torch.pow(10,(lag_tgt[:,3:4].clone()+u_ave))
                eulu_out, eulu_tgt = lag2eul(lag_out[:,:4], lag_tgt[:,:4], up_fac = args.mesh_up_fac, squeeze=True,rm_dis_mean=False)
                eps = 1e-2
                eulu_out = torch.div(eulu_out+eps,eul_out+eps)
                eulu_tgt = torch.div(eulu_tgt+eps,eul_tgt+eps)
        #        eulu_out[eulu_out!=eulu_out]=0 # handle zero-devision
        #        eulu_tgt[eulu_tgt!=eulu_tgt]=0
                eulu_out = torch.log10(eulu_out)
                eulu_tgt = torch.log10(eulu_tgt)
                output = torch.cat([eulu_out, output], dim=1)
                target = torch.cat([eulu_tgt, target], dim=1)                
                
                if batch == 1 and rank == 0:
                    print('lag2eul input shape :', input.shape, flush=True)
                    print('lag2eul target shape :', target.shape, flush=True)

            if args.cgan:
                output = torch.cat([input, output], dim=1)
                target = torch.cat([input, target], dim=1)
                if batch == 1 and rank == 0:
                    print('cgan target shape :', target.shape, flush=True)           

...
def train(
    ...
    for i, data in enumerate(loader):
        ...
        vel_lag_out, vel_lag_tgt = output[:, 5:], target[:, 5:]
        star_out, star_tgt = output[:, 4:5], target[:, 4:5]
       

        # Convert probabilities to binary 0 or 1 based on a threshold (0.5)
        #star_out = (star_out >= 0.5).float()
        u_out, u_tgt = output[:, 3:4], target[:, 3:4]
        ...
        star_loss = criterion(star_out, star_tgt)
        epoch_loss[10] += star_loss.detach()
        u_loss= criterion(u_out, u_tgt)
        epoch_loss[5] += u_loss.detach()
        total_loss+=star_loss
        total_loss+=u_loss
        if args.adv and epoch >= args.adv_start:
    ...
    if rank == 0:
        ...
        logger.add_scalar(
            "epoch/train/loss/energy", epoch_loss[5], global_step=epoch + 1
        )
        logger.add_scalar(
            "epoch/train/loss/star", epoch_loss[10], global_step=epoch + 1
        )
        ...
        if (epoch + 1) % 1 == 0:
            ...
            try:
                print(f"disp_lag_out min={disp_lag_out[-1].min().item()}, max={disp_lag_out[-1].max().item()}")
                print(f"disp_lag_tgt min={disp_lag_tgt[-1].min().item()}, max={disp_lag_tgt[-1].max().item()}")
                print(f"disp_diff min={(disp_lag_out[-1] - disp_lag_tgt[-1]).min().item()}, max={(disp_lag_out[-1] - disp_lag_tgt[-1]).max().item()}")

                print(f"disp_eul_out min={disp_eul_out[-1].min().item()}, max={disp_eul_out[-1].max().item()}")
                print(f"disp_eul_tgt min={disp_eul_tgt[-1].min().item()}, max={disp_eul_tgt[-1].max().item()}")
                print(f"eul_diff min={(disp_eul_out[-1] - disp_eul_tgt[-1]).min().item()}, max={(disp_eul_out[-1] - disp_eul_tgt[-1]).max().item()}")

                print(f"vel_lag_out min={vel_lag_out[-1].min().item()}, max={vel_lag_out[-1].max().item()}")
                print(f"vel_lag_tgt min={vel_lag_tgt[-1].min().item()}, max={vel_lag_tgt[-1].max().item()}")    
                print(f"vel_diff min={(vel_lag_out[-1] - vel_lag_tgt[-1]).min().item()}, max={(vel_lag_out[-1] - vel_lag_tgt[-1]).max().item()}")
                fig = plt_slices(
                    input[-1,:3],
                    disp_lag_out[-1],
                    disp_lag_tgt[-1],
